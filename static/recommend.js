$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = 'c312b42110c1fb3f770d1ff27b54c021';
    var title = $('.movie').val();
    if (title=="") {
...
function load_details(my_api_key,title){
  $.ajax({
    type: 'GET',
    url:'https://api.themoviedb.org/3/search/movie?api_key=c312b42110c1fb3f770d1ff27b54c021&query='+title,
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The movie searched does not exist in our database. Try another one."){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
...
function get_movie_details(movie_id,my_api_key,arr,movie_title) {
  $.ajax({
    type:'GET',
    url:'https://api.themoviedb.org/3/movie/'+movie_id+'?api_key=c312b42110c1fb3f770d1ff27b54c021',
